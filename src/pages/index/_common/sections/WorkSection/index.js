	...
	return (
		<Card as='section' variant='secondary' id='work' aria-labelledby='work-heading' className='scroll-mt-6 rounded-xl m-4 px-6 py-12 lg:px-10 -mt-4 lg:-mt-8 space-y-16'>
			<div className='w-full mx-auto max-w-full'>
				<p className='text-[.8em] uppercase tracking-[0.2em] font-semibold text-current/66'>My work</p>
				<LazyTerminalTypeEffect
					animationType='futuristic'
					element='h2'
					id='work-heading'
					className='mb-10 font-[Rubik] font-medium text-[clamp(1.25rem,5vw,2.5rem)] lg:text-[clamp(2rem,6vw,5rem)] tracking-tighter lg:leading-18 text-foreground'>
					Design to Production
				</LazyTerminalTypeEffect>
				<p className='text-lg leading-relaxed text-current/88 max-w-2xl'>
							...
							<AccordionItem className='flex flex-col gap-4' isExpanded={isExpanded} onToggle={() => toggleGroup(groupName)}>
								<AccordionTrigger icon='plus' gridCols='md:grid-cols-[minmax(0,1fr)_minmax(0,1.6fr)_24px] items-start' triggerPadding='p-0'>
									<div className='flex flex-col'>
										...
										<h3 className='font-[Rubik] text-[clamp(1.2rem,2.5vw,1.5rem)] leading-8 font-medium text-current mb-0'>{groupName}</h3>
									</div>
											...
											<div className='mt-6'>
												<Card variant='default' className='px-6 py-8 lg:px-10'>
													<div className='grid grid-cols-1 md:grid-cols-2 mb-6'>
														<div className='flex flex-col'>
															<p className='text-2xl font-[Rubik] font-medium text-foreground mb-0.5'>{selectedCase.title}</p>
															<p className='text-md text-current/66'>{selectedCase.subtitle}</p>
														</div>
														{(selectedCase.role || selectedCase.tools) ? (
															<div className='flex flex-col md:flex-row gap-x-4 gap-y-2 text-sm text-current/66 mt-2 justify-end ml-auto md:pl-6'>
