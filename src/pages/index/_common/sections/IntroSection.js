...
export const IntroSection = ({ onScrollToWork }) => {
	...
	return (
		<header className='relative w-full rounded-t-lg' aria-labelledby='hero-heading'>
			<section aria-labelledby='hero-heading' className='border-b border-border px-6 py-12 lg:px-12 lg:pb-16 lg:pt-14'>
				<div className='mx-auto grid max-w-full items-baseline-start gap-10 md:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)]'>
					<div className='space-y-6'>
						<div className='flex flex-wrap justify-inherit gap-3 text-current/44 items-center'>
						...
						<div className='max-w-[50rem] space-y-4 leading-relaxed text-current/66'>
							{Array.isArray(intro?.body) ? intro?.body.map((para, i) => (
								<p className='text-xl' key={i}>{para}</p>
							)) : <p className='text-xl'>{intro?.body}</p>}
							{intro?.detail && <p className='-mt-2.5 italic max-w-2xl'>{intro?.detail}</p>}
						</div>
