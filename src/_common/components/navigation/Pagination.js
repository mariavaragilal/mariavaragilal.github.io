...
const LINK_ACTIVE = 'border-border bg-background shadow-sm';
...
export const PaginationLink = ({ isActive = false, className = '', href, children, ...props }) => (
	<a
		...
		className={LINK_BASE + ' ' + (isActive ? LINK_ACTIVE : '') + ' ' + className}
		{...props}
	>{children}</a>
);

export const PaginationPrevious = ({ className = '', href, ...props }) => (
	<a href={href} aria-label='Go to previous page' className={LINK_BASE + ' gap-1 pl-2.5 ' + className} {...props}>
...
export const PaginationNext = ({ className = '', href, ...props }) => (
	<a href={href} aria-label='Go to next page' className={LINK_BASE + ' gap-1 pr-2.5 ' + className} {...props}>
		<span>Next</span>
		<span aria-hidden='true'>→</span>
	</a>
);
...
export const PaginationEllipsis = ({ className = '', ...props }) => (
	<span aria-hidden='true' className={'flex h-9 w-9 items-center justify-center ' + className} {...props}>…</span>
);
