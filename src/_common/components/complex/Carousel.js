...
export const Carousel = ({ orientation = 'horizontal', className = '', children, ...props }) => {
	...
	return (
		<CarouselContext.Provider value={{ ref, scrollPrev, scrollNext, canScrollPrev, canScrollNext, orientation, updateScrollState }}>
			<div role='region' aria-roledescription='carousel' className={'relative ' + className} {...props}>
				{children}
			</div>
	...
	return (
		<div className='overflow-hidden'>
			<div
				...
				aria-live='polite'
				className={'flex ' + (orientation === 'vertical' ? 'flex-col overflow-y-auto' : 'overflow-x-auto') + ' scroll-smooth snap-x snap-mandatory [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden -ml-4 ' + className}
				{...props}
			/>
		</div>
	);
...
export const CarouselItem = ({ className = '', ...props }) => (
	<div role='group' aria-roledescription='slide' className={'min-w-0 shrink-0 grow-0 basis-full pl-4 snap-start ' + className} {...props}/>
);
...
export const CarouselPrevious = ({ className = '', ...props }) => {
	...
	return (
		<button
			...
			className={BTN + ' ' + (orientation === 'vertical' ? '-top-12 left-1/2 -translate-x-1/2 rotate-90' : '-left-12 top-1/2 -translate-y-1/2') + ' ' + className}
			onClick={scrollPrev}
			{...props}
		>
...
export const CarouselNext = ({ className = '', ...props }) => {
	...
	return (
		<button
			...
			className={BTN + ' ' + (orientation === 'vertical' ? '-bottom-12 left-1/2 -translate-x-1/2 rotate-90' : '-right-12 top-1/2 -translate-y-1/2') + ' ' + className}
			onClick={scrollNext}
			{...props}
		>
			<span aria-hidden='true'>→</span>
