	...
	return (
		<SidebarContext.Provider value={{ state, open, setOpen, toggleSidebar }}>
			<div
				...
				className={cn('group/sidebar-wrapper flex min-h-svh w-full', className || undefined)}
				{...props}
			>
				{children}
			</div>
...
export const SidebarTrigger = ({ className = '', onClick, ...props }) => {
	...
	return (
		<button
			...
			className={cn('inline-flex h-7 w-7 items-center justify-center rounded-md text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground ' + focusRing, className)}
	...
	return (
		...
		>
			...
			<div className={cn('fixed inset-y-0 z-10 hidden h-svh w-[--sidebar-width] transition-[left,right,width] duration-200 ease-linear md:flex', side === 'left' ? 'left-0 group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)]' : 'right-0 group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)]', variant !== 'sidebar' ? 'p-2 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+theme(spacing.4)+2px)]' : 'group-data-[collapsible=icon]:w-[--sidebar-width-icon] group-data-[side=left]:border-r group-data-[side=right]:border-l')}>
				<div data-sidebar='sidebar' className={cn('flex h-full w-full flex-col bg-sidebar', variant === 'floating' ? 'rounded-lg border border-sidebar-border shadow' : variant === 'inset' ? 'rounded-xl' : '')}>
					{children}
				</div>
			</div>
		</div>
...
export const SidebarSeparator = ({ className = '', ...props }) => (
	<div data-sidebar='separator' className={cn('mx-2 w-auto h-px bg-sidebar-border', className)} {...props}/>
);
...
export const SidebarGroupLabel = ({ className = '', ...props }) => (
	<div data-sidebar='group-label' className={cn('duration-200 flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70 outline-none ring-sidebar-ring transition-[margin,opacity] ease-linear focus-visible:ring-2 group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0', className)} {...props}/>
);
...
export const SidebarMenuButton = ({ isActive = false, size = 'default', className = '', ...props }) => (
	<button
		...
		className={cn(MENU_BUTTON_BASE, className)}
		{...props}
	/>
);

export const SidebarMenuBadge = ({ className = '', ...props }) => (
	<div data-sidebar='menu-badge' className={cn('absolute right-1 flex h-5 min-w-5 items-center justify-center rounded-md px-1 text-xs font-medium tabular-nums text-sidebar-foreground select-none pointer-events-none', className)} {...props}/>
);

export const SidebarMenuSub = ({ className = '', ...props }) => (
	<ul data-sidebar='menu-sub' className={cn('mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border px-2.5 py-0.5 group-data-[collapsible=icon]:hidden', className)} {...props}/>
);
...
export const SidebarRail = ({ className = '', ...props }) => {
	...
	return (
		<button
			...
			className={cn('absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 cursor-w-resize transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-sidebar-border group-data-[side=left]:-right-4 sm:flex', className)}
			{...props}
		/>
	);
};
